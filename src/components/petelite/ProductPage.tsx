...
import { getProductBySlug, formatPrice, calculateSavings, getRelatedProducts } from '@/lib/data';
    ...
    <AnimatePresence>
      ...
      >
        ...
        >
          ...
          <button
            onClick={onClose}
            className="absolute top-4 right-4 z-10 p-2 bg-white/90 backdrop-blur-sm rounded-full shadow-sm hover:shadow-md transition-shadow text-black"
          ...
          <div className="flex-1 overflow-y-auto">
            {/* Breadcrumbs */}
            <div className="hidden sm:flex items-center gap-1.5 px-6 lg:px-10 pt-5 text-xs text-black/40">
              <a href="#" className="hover:text-brand transition-colors" onClick={onClose}>Home</a>
            ...
            <div className="grid lg:grid-cols-2 gap-6 lg:gap-10 xl:gap-14 px-5 sm:px-6 lg:px-10 py-6 sm:py-8">
              ...
              <div>
                {/* Badge */}
                {product.badge && (
                  <span className="inline-block px-2.5 py-1 text-[10px] font-bold tracking-wider uppercase rounded bg-brand text-white mb-3">
                    {product.badge}
                  </span>
                )}
                ...
                <div className="mt-8 space-y-3">
                  <div className="flex items-center gap-4">
                    <div className="flex items-center border border-warm-border rounded-lg">
                      ...
                      <span className="px-4 text-sm font-medium text-black min-w-[2.5rem] text-center">
                        {quantity}
                      </span>
                      <button
                  ...
                  <button
                    onClick={() => addItem(product, quantity)}
                    className="w-full py-3.5 text-sm font-semibold tracking-wide uppercase bg-brand-dark text-white rounded-lg hover:bg-[#2E422D] transition-all duration-300 hover:-translate-y-px"
                  >
                    BUY IT NOW
                  </button>
                </div>
                ...
                {/* Trust signals */}
                <div className="mt-6 flex flex-wrap gap-x-4 gap-y-2 text-xs text-black/40">
              ...
              <div className="flex gap-6 sm:gap-8 border-b border-warm-border overflow-x-auto scrollbar-hide">
                ...
                {product.faqs && product.faqs.length > 0 && (
                  <button
                    ...
                    className="pb-3 text-sm font-medium whitespace-nowrap text-black/40 border-b-2 border-transparent hover:text-black transition-colors -mb-px"
                    style={{ display: 'none' }}
                  >
                    FAQ
                  </button>
                  ...
                  <ul className="space-y-3">
                    {product.benefits.map((b) => (
                      <li key={b} className="flex items-start gap-3">
                        ...
                        <span className="text-sm sm:text-base text-black leading-relaxed">{b}</span>
                      </li>
                    ))}
                  </ul>
                )}
                {activeTab === 'details' && (
                  <div className="space-y-0">
                    {Object.entries(product.specifications).map(([key, value]) => (
                      ...
                      >
                        <span className="text-sm font-medium text-black/40">{key}</span>
                  ...
                  <ul className="space-y-2">
                    {product.included.map((item) => (
                      <li key={item} className="flex items-start gap-2.5">
                        <div className="w-4 h-4 rounded-full bg-brand/10 flex items-center justify-center flex-shrink-0 mt-0.5">
                  ...
                  <ol className="space-y-4">
                    {product.howItWorks.map((step, i) => (
                      <li key={i} className="flex items-start gap-4">
                        <span className="flex-shrink-0 w-8 h-8 rounded-full bg-brand/10 flex items-center justify-center text-sm font-semibold text-brand">
                          {i + 1}
                        </span>
                        <span className="text-sm text-black leading-relaxed pt-1">{step}</span>
                      </li>
                    ))}
                  </ol>
                )}
                  ...
                  {product.faqs.map((faq, i) => (
                    <div key={i} className="border-b border-warm-border">
                      ...
                      >
                        ...
                        <ChevronRight
                          className={`w-4 h-4 text-black/40 flex-shrink-0 transition-transform ${faqOpen === i ? 'rotate-90' : ''}`}
                        />
                      </button>
                      {faqOpen === i && (
                        <motion.div
            ...
            <div className="border-t border-warm-border px-5 sm:px-6 lg:px-10 py-8 sm:py-10">
              <h2 className="text-xl font-bold text-black mb-2">Customer Reviews</h2>
              ...
              <p className="text-sm text-black/40 bg-brand-light/30 rounded-lg p-6 text-center">
